...
setwd(("C:/Users/Dan/Documents/GitHub/CompetitionTask"))
...
data <- read.csv("C:/Users/Dan/Dropbox/Monash/My Papers/2014 JoN_Cog_battery_DAT1_VNTR/New_data_from_Beth/AllParticipantData.csv", header=T)
...
#Remove trials where RT too slow 
data<-data[!data$RT<200,]
#....or too fast to be real RTs
...
# light this "(1 | Site/ID) - which means ID is nested inside 
...
summary(data2$Site) #Once you kick out the NA cells, there are currently no Monash 
#participant with left - i.e. currently no Monash genotypes 
...
#Plot RT TargetLoc x dat1utr: DAT1UTR_by_TargetLoc p=5.184e-05 ***
source("summarySE.R") 
source("summarySEwithin.R") #function to calculate Std.Er of mean
source("normDataWithin.R")
...
#SetSize_by_TargetLoc_by_DAT1UTR, p= 0.041838 *
#make a SetSizebyTargetLoc factor:
data2$SetSizebyTargetLoc<-interaction(data2$TargetLoc,data2$SetSize)
source("summarySE.R") 
source("summarySEwithin.R") #function to calculate Std.Er of mean
source("normDataWithin.R")
plotdata <- summarySEwithin(data2, measurevar="RT", withinvars=c("SetSizebyTargetLoc"), betweenvars="dat1utr", idvar="ID")
ggplot(plotdata, aes(x=SetSizebyTargetLoc, y=RT, fill=dat1utr)) +
    geom_bar(position=position_dodge(.9), colour="Black", stat="identity") + 
    geom_errorbar(position=position_dodge(.9), width=.3, aes(ymin=RT-ci, ymax=RT+ci)) + #can change "se" to "ci" if I want to use 95%ci instead
    geom_hline(yintercept=0) +  coord_cartesian(ylim = c(450, 900)) +
    xlab("TargetLoc by Setsize") + ylab("Reaction-time (ms)") +
    theme(axis.title.x = element_text(face="bold", size=12),
          axis.text.x  = element_text(face="bold", angle=0,  size=12)) +
    theme(axis.title.y = element_text(face="bold", size=12),
          axis.text.y  = element_text(angle=0, vjust=0.5, size=12)) +
...
#TargetLoc_by_DisplayType_by_DAT1UTR, p= 0.042358 *
#make a SetSizebyTargetLoc factor:
data2$DisplayTypebyTargetLoc<-interaction(data2$TargetLoc,data2$DisplayType)
source("summarySE.R") 
source("summarySEwithin.R") #function to calculate Std.Er of mean
source("normDataWithin.R")
plotdata <- summarySEwithin(data2, measurevar="RT", withinvars=c("DisplayTypebyTargetLoc"), betweenvars="dat1utr", idvar="ID")
ggplot(plotdata, aes(x=DisplayTypebyTargetLoc, y=RT, fill=dat1utr)) +
    geom_bar(position=position_dodge(.9), colour="Black", stat="identity") + 
    geom_errorbar(position=position_dodge(.9), width=.3, aes(ymin=RT-ci, ymax=RT+ci)) + #can change "se" to "ci" if I want to use 95%ci instead
    geom_hline(yintercept=0) +  coord_cartesian(ylim = c(450, 900)) +
    xlab("TargetLoc by Display-type") + ylab("Reaction-time (ms)") +
    theme(axis.title.x = element_text(face="bold", size=12),
          axis.text.x  = element_text(face="bold", angle=0,  size=12)) +
    theme(axis.title.y = element_text(face="bold", size=12),
          axis.text.y  = element_text(angle=0, vjust=0.5, size=12)) +
...
###### not every position has observations accross all of the other vatiables, lets explore:

# Calculate Set-size effect by TargetLoc
require(reshape2)
#Bring SetSize Factor up into wide formay to and calculate SetSize effect
DF_TargetLoc_wide <- dcast(DF_collapsed, ID + dat1utr + DisplayType + TargetLoc ~ SetSize, value.var="RT")
#Calculate SetSize effect for each Targloc:
DF_TargetLoc_wide$SetSizeEffect<-DF_TargetLoc_wide$Eight - DF_TargetLoc_wide$Four
#Only keep TargLocs that we can actually calculate setsize effects for:
SetSizeEffect_CompleteCases<-DF_TargetLoc_wide[complete.cases(DF_TargetLoc_wide$SetSizeEffect),]
#Check which TargLocs we can actually calculate setsize effects for:
summary(SetSizeEffect_CompleteCases$TargetLoc) #So this shows that we can not calculate 
                                              #Setsize effect for all 16 target locations
                                              # That will be why the model was "rank deficient"


###Now try model DAT1 x TargetLoc in the collapsed setsize-effect data
random_intercepts_only_SetSizeEffect<-lmer(SetSizeEffect ~ 1 + (1 | dat1utr/ID) +(1|TargetLoc), data = DF_TargetLoc_wide, na.action = na.omit, REML=FALSE)
TargetLoc_SetSizeEffect<-update(random_intercepts_only_SetSizeEffect, .~. + TargetLoc)
DAT1UTR_SetSizeEffect<-update(TargetLoc_SetSizeEffect, .~. + dat1utr)
TargetLoc_by_DAT1UTR_SetSizeEffect<-update(DAT1UTR_SetSizeEffect, .~. + TargetLoc*dat1utr)

anova(random_intercepts_only_SetSizeEffect,TargetLoc_SetSizeEffect,DAT1UTR_SetSizeEffect, TargetLoc_by_DAT1UTR_SetSizeEffect)

require(effects)
eff.TargetLoc_by_DAT1UTR_SetSizeEffect <- Effect(c("TargetLoc", "dat1utr"), TargetLoc_by_DAT1UTR_SetSizeEffect)
plot(eff.TargetLoc_by_DAT1UTR_SetSizeEffect, layout=c(3,1), main=NULL, rug=F, multiline =F, alternating=F, ticks.x=NULL)
